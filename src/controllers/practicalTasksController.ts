 ...
 */
export const getPracticalTasksByTechnologyHandler = asyncHandler(async (req: Request, res: Response) => {
  const { technologyName } = req.params;
  const googleToken = getGoogleTokenFromRequest(req);
  const tasks = await getPracticalTasksByTechnology(technologyName, googleToken);
  return sendSuccess(res, tasks);
});
 ...
 */
export const addPracticalTaskHandler = asyncHandler(async (req: Request, res: Response) => {
 ...
 */
export const updatePracticalTaskHandler = asyncHandler(async (req: Request, res: Response) => {
  setUserCredentials(req.googleToken!);
 ...
 */
export const deletePracticalTaskHandler = asyncHandler(async (req: Request, res: Response) => {
  setUserCredentials(req.googleToken!);
  const { technologyName, rowIndex } = req.params;
 ...
 */
export const reorderPracticalTasksHandler = asyncHandler(async (req: Request, res: Response) => {
  setUserCredentials(req.googleToken!);
  const { technologyName } = req.params;
  const { oldIndex, newIndex } = req.body;

  if (typeof oldIndex !== 'number' || typeof newIndex !== 'number') {
    return sendValidationError(res, 'oldIndex and newIndex are required and must be numbers');
  }
